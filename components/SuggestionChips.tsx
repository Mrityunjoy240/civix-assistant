...
export default function SuggestionChips({ flows, suggestions, onSelect, disabled }: SuggestionChipsProps) {
  const normalizedFlows = flows ?? (suggestions ?? []).map((s) => ({ label: s, prompt: s, icon: '💡' }));
  if (normalizedFlows.length === 0) return null;

  return (
    <div className="flex flex-wrap gap-2 mb-6">
      {normalizedFlows.map((flow, index) => (
        <motion.button
          key={index}
          whileHover={{ y: -2 }}
          whileTap={{ scale: 0.95 }}
